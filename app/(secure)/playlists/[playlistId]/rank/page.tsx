...
export default function WamPlaylistRankPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    fetch(`/api/playlists/${encodeURIComponent(playlistId)}/ranking`, {
      signal: ac.signal,
    })
      .then(async (res) => {
  ...
  return (
    <PlaylistRankingView data={data} backHref="/playlists" backLabel="Playlists" />
  );
}
